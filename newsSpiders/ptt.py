...
class DiscoverSite:
    ...
    def crawl_and_insert(self, article_urls):
        for url in article_urls:
            ...
            next_snapshot_at = generate_next_snapshot_time(
                self.site_info["url"], snapshot_count=1, snapshot_time=now
            )
    ...
    def run(self, depth):
        ...
        dedup_limit = 20 * 1.5 * (depth + int(buffer_pages))
        recent_urls_in_db = [
            x["url"]
            for x in self.queries.get_recent_articles_by_site(
                site_id=self.site_info["site_id"], limit=dedup_limit
            )
        ]
        extra_pages = 0
        d = 0
        page_url = self.site_info["url"]
        stop = False
