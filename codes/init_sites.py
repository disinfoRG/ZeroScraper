...
for original_id, site in sites.items():
    if 'config' in site:
        sql.write("INSERT INTO `Site`(type, name, url, config) VALUES('{0}', '{1}', '{2}', '{3}');\n".format(site['type'], site['name'], site['url'], json.dumps(site['config'])))
    else:
        sql.write("INSERT INTO `Site`(type, name, url) VALUES('{0}', '{1}', '{2}');\n".format(site['type'], site['name'], site['url']))
